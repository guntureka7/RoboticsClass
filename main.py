# Guntur Eka Wardana_1103174102_Tugas 1_MicroProject Help The TurtleBot
...
# ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
# Mengimpor berbagai modul dan kelas dari ROS
# dan pustaka Python lainnya, yang digunakan di seluruh skrip.

class RobotControl():
    # Ini mendefinisikan kelas Python bernama RobotControl,
    # yang mengenkapsulasi fungsionalitas untuk mengendalikan robot.

    def __init__(self):
        # Ini adalah metode konstruktor untuk kelas RobotControl. Metode ini menginisialisasi berbagai atribut, mengatur penerbit dan pelanggan ROS,
        # dan melakukan tugas penyiapan lain ketika sebuah instance kelas dibuat.
    ...
    def laser_callback(self,
                       msg):  # Metode-metode ini adalah fungsi callback yang menangani data yang masuk dari sensor dan odometri.
        self.laser_msg = msg

    def summit_laser_callback(self,
                              msg):  # Metode-metode ini adalah fungsi callback yang menangani data yang masuk dari sensor dan odometri.
    ...
    def move_straight_time(self, motion, speed, time):
        ...
        i = 0
        Perulangan  # untuk mempublikasikan estimasi kecepatan, current_distance = kecepatan * (t1 - t0)
        while (i <= time):
            # Publish the velocity
            self.vel_publisher.publish(self.cmd)
            self.summit_vel_publisher.publish(self.cmd)
        ...
        # atur kecepatan ke nol untuk menghentikan robot
        self.stop_robot()
...
if __name__ == '__main__':
    # Pernyataan bersyarat ini memeriksa apakah skrip dijalankan sebagai program utama (tidak diimpor sebagai modul).
    # rospy.init_node('robot_control_node', anonymous=True)
